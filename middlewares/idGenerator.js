...
exports.staffIdGenerator = async (req, res, next) => {
  ...
  if (getStaff.length < 1) req.body.username = 'NUT/ADMIN/001'
  else {
    ...
    req.body.username = `NUT/STAFF/${id[2] < 10 && parseInt(id[2]) + 1 < 10 ? '00' : id[2] >= 99 ? '' : '0'}${parseInt(id[2]) + 1}`
  }
...
exports.studentIdGenerator = async (req, res, next) => {
  ...
  if (getStudent.length < 1) req.body.username = `NUT/${splitYear}/001`
  else {
    // 3. if there are staffs get the last id and icrement by one
    ...
    req.body.username = `NUT/${splitYear}/${id[2] < 10 && parseInt(id[2]) + 1 < 10 ? '00' : id[2] >= 100 ? '' : '0'}${parseInt(id[2]) + 1}`
  }
